...
const CurrentTimeBox = ({ title, hoursLeft, minutesLeft, secondsLeft, isRunning, isPaused, progressInPercent, pausesCount, handleStart, handleStop, togglePause, isEditable, onEdit }) => {
    return(
        <div className={`CurrentTimeBox ${isEditable ? "inactive" : ""}`}>
        <h1>{title}</h1>
        <Clock hours={hoursLeft} minutes={minutesLeft} seconds={secondsLeft} inactive={isPaused ? "inactive" : ""} />
